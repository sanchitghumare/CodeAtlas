...
import { ArrowRight, FileSearch, GitBranch, ShieldCheck, GitPullRequest, SearchCode } from "lucide-react";
...
function TerminalPreview() {
  return (
    <div className="overflow-hidden rounded-xl border border-[#30363D] bg-[#161B22] shadow-sm">
      <div className="flex h-10 items-center gap-2 border-b border-[#30363D] px-4">
      ...
      <div className="space-y-4 p-6 font-mono text-sm leading-7">
        <p className="text-[#3B82F6]">
          &gt; Why is this endpoint marked as critical?
        </p>

        <p className="text-[#F0F6FC]">
          Missing authorization boundary detected.
        </p>
        ...
        <div className="rounded-md border border-[#30363D] bg-[#0D1117] p-3">
          <p className="mb-2 text-xs text-[#238636]">
            Suggested Fix
          </p>

          <pre className="overflow-x-auto text-xs text-[#F0F6FC]">
    ...
    <main className="min-h-screen bg-[#0D1117] text-[#F0F6FC]">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <header className="flex h-16 items-center justify-between border-b border-[#30363D]">
          <Link href="/" className="flex items-center gap-2.5 font-semibold tracking-tight" aria-label="CodeAtlas home">
            <span className="grid size-7 place-items-center rounded-md bg-[#3B82F6] text-white"><GitPullRequest className="size-4" /></span>
            <span>CodeAtlas</span>
          </Link>
        ...
        <section className="grid gap-12 py-18 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-16 lg:py-28">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-5 text-balance text-4xl font-semibold tracking-[-0.045em] sm:text-6xl lg:text-[4.2rem] lg:leading-[1.02]">Ship better code with intelligent repository reviews.</h1>
            <p className="mt-6 max-w-xl text-pretty text-base leading-7 text-[#8B949E] sm:text-lg">CodeAtlas analyzes your repository, connects findings across files, and gives you a report that is ready for an engineering decision.</p>
            <div className="mt-9 flex flex-wrap items-center gap-3">
              ...
              <a href="login" className="inline-flex h-10 items-center gap-2 rounded-md border border-[#30363D] px-4 text-sm font-medium text-[#8B949E] transition-colors hover:border-[#8B949E] hover:text-[#F0F6FC]">Sign in with GitHub <GitBranch className="size-4" /></a>
            </div>
